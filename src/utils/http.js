...
function getInstances(conf) {
  const result = {}
  const domains = conf.root || null
  const opts = merge(axios.defaults)
  // eslint-disable-next-line
  for (const ky in opts) {
...
//   if (conf.method === 'post') {
//     const userInfo = router.app.$store.state.userInfo || {
//       STAFF_ID: '',
//       GLOBAL_SESSION_ID: '',
//       GLOBAL_SIGN: ''
//     }
...
http.$base.interceptors.request.use(requestInterceptor, requestError)
http.$base.interceptors.response.use(responseInterceptor, responseError)

export const { $base } = http
// export const { $testurl } = http
